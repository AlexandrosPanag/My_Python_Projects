...
#!/usr/bin/env python3
"""
WebM Resolution Doubler with Subtle Sharpening
Doubles the resolution of a WebM video and applies a gentle unsharp mask.
"""
...
def double_resolution(input_file, output_file='output.webm'):
    """
    Double the resolution of a WebM video with subtle sharpening.
    ...
    # FFmpeg filter chain:
    # 1. scale: Lanczos scaling (high quality) to double resolution
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python webm_doubler.py input.webm [output.webm]")
        print("\nExample:")
        print("  python webm_doubler.py input.webm")
        print("  python webm_doubler.py input.webm output.webm")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'output.webm'
    
    double_resolution(input_file, output_file)