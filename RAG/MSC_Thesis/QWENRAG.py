...
║ Tertiary: Use Qwen2-1.5B AI to generate an answer ║
...
║ ✓ Local AI fallback (Qwen2-1.5B - runs offline) ║
...
print("\n" + "="*70)
print("🚀 RAG SYSTEM STARTUP (Qwen2-1.5B Edition)")
print("="*70)
...
#
╔══════════════════════════════════════════════════════════════════════════════╗
# ║ OPTIONAL AI LANGUAGE MODEL (Qwen2-1.5B) ║
# ║ ║
...
from transformers import pipeline, AutoTokenizer # High-level API for
easy model usage
import torch # PyTorch - the deep learning framework
# Show loading message
print("Loading Qwen2-1.5B-Instruct model (fast, multilingual AI)...")
# Model name - Qwen2 has excellent multilingual support (English +
Greek)
AI_MODEL_NAME = "Qwen/Qwen2-1.5B-Instruct"
...
model=AI_MODEL_NAME, # Qwen2-1.5B - excellent multilingual support
tokenizer=ai_tokenizer, # Use our loaded tokenizer
torch_dtype=torch.bfloat16, # Use bfloat16 for memory efficiency
device_map="auto", # Auto-detect GPU/CPU
)
# Loading succeeded!
USE_AI_FALLBACK = True
print("✅ Qwen2-1.5B-Instruct loaded successfully! AI-powered answers
enabled.")
except ImportError:
...
INDEX_FILE = "vector_store.pkl"
print("[3/4] Loading knowledge base...")
...
print(f" ✓ Index built and saved! ({len(document_chunks)} chunks
indexed)")
else:
print(" ⚡ Loading cached index (much faster!)...")
...
print(f" ✓ Index loaded from cache! ({len(document_chunks)} chunks)")
#
╔══════════════════════════════════════════════════════════════════════════════╗
...
# ║ │ ↓ (if no match) │ ║
# ║ │ 3. 🤖 AI LANGUAGE MODEL (Qwen2-1.5B) │ ║
...
if language == 'en':
# Keep only English docs (from /en/ folder or files NOT containing
Greek characters)
filtered_chunks = [(chunk, score) for chunk, score in chunks
if '/en/' in chunk['source'] or (not any(c in
'αβγδεζηθικλμνξοπρστυφχψω' for c in chunk['source'].lower()))]
elif language == 'greeklish':
# Keep only Greeklish docs (from el_greeklish folder)
filtered_chunks = [(chunk, score) for chunk, score in chunks
if 'el_greeklish' in chunk['source']]
else:
# Keep only Greek docs (from /el/ folder but not el_greeklish)
filtered_chunks = [(chunk, score) for chunk, score in chunks
if '/el/' in chunk['source'] and 'el_greeklish' not in
chunk['source']]
...
"""
Generate an answer using the Qwen2-1.5B-Instruct AI model.
This is used as a fallback when documents and FAQs don't have an
answer.
The AI can provide general knowledge responses.
WHY QWEN2?
• Excellent multilingual support (English + Greek)
• Better instruction-following than Qwen1.5
• Similar size (1.5B params) - runs on CPU/modest GPU
• Open source (Apache 2.0 license)
HOW IT WORKS:
1. Build a prompt with available context from document chunks
2. Ask Qwen2 to answer based on the context
...
# ─── CREATE PROMPT USING QWEN2 CHAT TEMPLATE ───
# Qwen2 uses a message-based format with roles
if language == 'en':
system_msg = "You are a helpful assistant for Open eClass LMS. Answer
briefly and accurately in 2-3 sentences. If you don't know the answer,
say so honestly."
user_msg = f"Context: {context}\n\nQuestion: {user_input}"
else:
system_msg = "Είσαι βοηθός για το Open eClass. Απάντησε σύντομα και
ακριβώς σε 2-3 προτάσεις. Αν δεν ξέρεις την απάντηση, πες το
ειλικρινά."
user_msg = f"Πλαίσιο: {context}\n\nΕρώτηση: {user_input}"
...
prompt = ai_tokenizer.apply_chat_template(
messages,
tokenize=False,
...
prompt,
max_new_tokens=100, # Limit response length (slightly more than before)
...
# Remove any trailing special tokens that Qwen might add
...
# ═══ STEP 4: TRY AI MODEL (Qwen2-1.5B) ═══
if USE_AI_FALLBACK:
print(f"[AI] Trying Qwen2-1.5B AI model...")
...
# Show AI status
print(f"🤖 AI Fallback: {'✅ Qwen2-1.5B-Instruct Enabled' if
USE_AI_FALLBACK else '❌ Disabled'}")
print("="*70)
...
print(" 5️⃣ Falls back to FAQ if no documents match (EN↔EL
cross-lingual)")
print(" 6️⃣ Falls back to Qwen2 AI for general knowledge queries")
# ─── SHOW HELPFUL HINTS ───
